...
def is_different_from_old_password(old_pass: str, new_pass: str) -> bool:
    ...
    """
    # check common letter, digits, anything
    for i in range(len(old_pass.lower())):
        # + 1 since in [] the end letter is EXCLUSIVE, range(start, stop)
        for j in range(i + 1, len(old_pass.lower()) + 1):
            sub = old_pass[i:j].lower()
            print(i, j, sub)
            sub_reversed = sub[::-1].lower()
            if sub in new_pass.lower() or sub_reversed in new_pass.lower():
                if len(sub) >= (len(new_pass) / 2) or len(sub_reversed) >= (len(new_pass) / 2):
                    return False
    return True
...
def is_name_in_password(password: str, name: str) -> bool:
    ...
    for check in name_split:
        if check in password.lower() or check in password[::-1].lower():
            # print("ch", check)
            return True
    return False
...
def is_birthday_in_password(password: str, birthdate: str) -> bool:
    ...
    print(password, birthdate, date)
    if date[2][::-1] in password or any(i in password for i in date) or date[2][2:] in password:
        return True
    else:
        return False
...
def is_password_valid(new_password: str, old_password: str, name: str, birthdate: str) -> bool:
    ...
    """
    if is_correct_length(new_password) and includes_uppercase(new_password) and includes_lowercase(new_password) and includes_special(new_password) and includes_number(new_password) and not is_birthday_in_password(new_password, birthdate) and not is_name_in_password(new_password, name) and is_different_from_old_password(old_password, new_password):
        return True
    else:
        return False
