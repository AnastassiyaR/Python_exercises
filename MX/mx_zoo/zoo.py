...
def parse_animal(animal_str: str) -> list:
    ...
    return [species, scientific_name, age_up_to, [min_weight, max_weight], [min_height, max_height], diet, habitat]
...
def animals_starting_with(animal_data: list, letter: str) -> list:
    ...
    """
    return sorted(map(lambda animal: animal[0], filter(lambda animal: animal[0].startswith(letter), animal_data)))
...
def find_how_many_pumpkins_are_needed_to_feed_animals(animal_data: list) -> int:
    ...
    return functools.reduce(
        lambda total, animal: total + (((animal[3][0] + animal[3][1]) / 2) * 0.06 / 3) * 90 * 2,
        filter(lambda animal: animal[5] in ["herbivorous", "omnivorous"], animal_data), 0
    )
...
def total_noise_level(animal_data: list) -> float:
    ...
    """
    return sum(map(lambda animal: sum(animal[3]) / 2 * 0.01, animal_data))
...
def zoo_parade_length(animal_data: list) -> float:
    ...
    """
    return sum(map(lambda animal: sum(animal[4]) / 2, animal_data))
...
def animal_olympics_winner(animal_data: list) -> str:
    ...
    """
    # Эта функция находит элемент с минимальным значением, основываясь на ключе, который вы передаете.
...
def total_feather_count(animal_data: list) -> float:
    ...
    """
    return sum(map(lambda animal: sum(animal[3]) / 2 * 1000, animal_data))
...
def zoo_weight_on_other_planet(animal_data: list) -> float:
    ...
    """
    return sum(map(lambda animal: sum(animal[3]) / 2 * 0.5, animal_data))
...
def sort_alphabetically_by_scientific_name(animal_data: list) -> list:
    ...
    """
    return sorted(map(lambda animal: (animal[0], animal[1]), animal_data), key=lambda animal: animal[1])
    # sorted(iterable, key=None, reverse=False) спроси почему for
...
def find_animals_whose_height_is_less_than(animal_data: list, height_limit: float) -> list:
    ...
    """
    return [animal[0] for animal in sorted(filter(lambda animal: animal[4][1] < height_limit, animal_data), key=lambda animal: animal[4][1])]

    # return sorted(map(lambda animal: animal[0], filter(lambda animal: animal[4][1] < height_limit, animal_data)), key= lambda animal: animal[4])

    # map(function, iterable) for processing elements
...
def filter_animals_based_on_diet(animal_data: list, diet: str) -> list:
    ...
    """
    return sorted(map(lambda animal: animal[0], filter(lambda animal: animal[5] == diet, animal_data)))
...
def create_animal_descriptions(animal_data: list) -> list:
    ...
    """
    return list(map(lambda animal: (
        f"{animal[0]} ({animal[1]}) lives in {animal[6]} and its diet is {animal[5]}. "
        f"These animals can live up to {animal[2]} years, and they weigh between {animal[3][0]} "
        f"kg and {animal[3][1]} kg as adults."
    ), animal_data))
...
def calculate_ecological_impact_score(animal_data: list) -> float:
    ...
    return functools.reduce(lambda total, animal: total + (10 + (0.001 * (animal[3][0] + animal[3][1]) / 2)) * diet_factor.get(animal[5].lower(), 1) + habitat_factor.get(animal[6].lower(), 0), animal_data, 0)
